# Aggregate actors by averaging their weights.
import torch
import copy
import os
os.chdir('..')
...
def get_actors(problem="gisp"):
    ...
    for seed in os.listdir(folder):
        ...
        actor_wrapper = NeuralNetworkWrapper()
        actor_wrapper.load_checkpoint(folder=fold_seed, filename="weights")
        actors.append(actor_wrapper.nnet)
    return actors

def average_actors(actors, problem="gisp"):
    avg_actor = copy.deepcopy(actors[0])
    state_dicts = [actor.state_dict() for actor in actors]

    averaged_dict = {}
    for key in state_dicts[0]:
        averaged_dict[key] = sum([sd[key].float() for sd in state_dicts]) / len(actors)

    avg_actor.load_state_dict(averaged_dict)
    ...
    avg_actor_wrapper = NeuralNetworkWrapper(glob_actor=avg_actor)  # si tu as une classe qui encapsule le modèle
    avg_actor_wrapper.save_checkpoint(folder=folder, filename=f"global_{problem}_actor.pth.tar")
...
if __name__ == "__main__":
    ...
    parser.add_argument('test', type=bool, help='wether to test the actor or not')
    args = parser.parse_args()
    actors = get_actors(problem=args.problem)
    average_actors(actors, problem=args.problem)
    if args.test:
        ...
        for seed in range(15):
            for i in range(1):
                ...
                final_simulation_folder = os.path.join(simulation_folder, f"seed__{seed}", f"_loop__{i}")
                evaluation_gp(args.problem, n_test_instances=n_test_instances, gp_func_dic=gp_func_dic, 
                                saving_folder=final_simulation_folder, RL=True, inputs_type="only_scores")
                
                # Gather information from JSON files for the specified problems and partitions
                dic_info = gather_info_from_json_files(problems=[args.problem], partitions=["test"], saving_folder=final_simulation_folder)
                list_dict.append(dic_info)
                # Display the output results
                just_get_the_output_results(list_dict, several_dicts=True)
