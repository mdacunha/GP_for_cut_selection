...
import learning_to_comparenodes
from learning_to_comparenodes.learning.data_type import BipartiteGraphPairData
...
class CompFeaturizerSVM():
    ...
    def get_features(self, node):
        ...
        f = []
        #feat[depth]
        f.append(node.getDepth())

        #feat[val]
        f.apppend(node.getLowerbound()/self.getDualboundRoot())
        if model.isEQ(upperbound, lowerbound):
            f.append(0)
        elif model.isZero(lowerbound):
            f.append(0)
        else:
            f.append((upperbound - lowerbound) / abs(lowerbound))
...
"""def getHeHeaumeEisnerFeatures(node,model, depth ):#model.getDepth()+1
