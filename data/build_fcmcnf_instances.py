...
def generate_fcmcnf(rng, filename, n_nodes, n_commodities, c_range, d_range, k_max, ratio, er_prob):
    ...
    with open(filename, 'w') as file:
        file.write("minimize\nOBJ:")
        file.write("".join(
            [f" + {commodities[k][2] * adj_mat[i][j][0]}x_{i + 1}_{j + 1}_{k + 1}" for (i, j) in edge_list for k in
             range(n_commodities)]))
        ...
        for i in range(n_nodes):
            for k in range(n_commodities):
                delta_i = 1 if (commodities[k][0] == i) else (
                    -1 if commodities[k][1] == i else 0)  # 1 if source, -1 if sink, 0 if else
        ...
        for (i, j) in edge_list:
            file.write(f"arc_{i + 1}_{j + 1}:" +
                       "".join([f" +{commodities[k][2]}x_{i + 1}_{j + 1}_{k + 1}" for k in
                                range(n_commodities)]) + f"-{adj_mat[i][j][2]}y_{i + 1}_{j + 1} <= +0\n")
...
def generate_capacited_facility_location(rng, filename, n_customers, n_facilities, ratio=1):
    ...
    with open(filename, 'w') as file:
        ...
        for i in range(n_customers):
            file.write(
                f"demand_{i + 1}:" + "".join([f" -1x_{i + 1}_{j + 1}" for j in range(n_facilities)]) + f" <= -1\n")
...
def generate_instances(n, min_n_nodes, max_n_nodes, min_n_commodities, max_n_commodities, er_prob,
                       lp_dir, solveInstance):
    for i in range(n):
        solved = False
        while solved is False:
            ...
            if solveInstance:
                ...
                #print(model.getNNodes())   ###############only for training
...
if __name__ == "__main__":
    ...
    generate_instances(n,min_n, max_n, min_n_commodities, max_n_commodities, er_prob,
                       lp_dir, True)
